...
router.put('/:id', async (request,response) => {
    try{
        if(
            ...
            return response.status(400).send({messege : "All Fields are required"});
        }
        const {id} = request.params;
        const result = await Book.findByIdAndUpdate(id,request.body);
        if(!result){
            return response.status(404).json({messege : "Book is not Found"});

        } 
        return response.status(200).send({messege: "Book Updated Successfully"});
    } catch(error){
        console.error(error);
        response.status(500).send({message: error.message});
    }
